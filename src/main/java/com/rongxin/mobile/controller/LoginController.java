...
public class LoginController extends BaseController {
    ...
    @ResponseBody
    @ApiOperation("手机号码登录，用户名、手机号、密码、验证码、登录方式：0验证码登录，1用户名密码登录，2手机登录，3微信登录")
    public AjaxResult loginByPassword(@RequestBody LoginBody loginBody) {
        // 登录结果
        LoginParams loginParams = new LoginParams();
        loginParams.setUsername(loginBody.getUsername());
        loginParams.setPassword(loginBody.getPassword());
        loginParams.setPhoneNo(loginBody.getPhonenumber());
        loginParams.setValidCode(loginBody.getCode());
