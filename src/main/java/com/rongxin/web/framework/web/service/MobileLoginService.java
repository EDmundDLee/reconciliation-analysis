...
//        }
    ...
    public AjaxResult wxOpenIdlogin(LoginParams loginParams) {
        log.debug("login and loginParams:{}", loginParams);
        ...
        try
        {
            ...
            if("3".equals(loginParams.getLoginType()))
            {
                authentication = authenticationManager.authenticate(new WxCodeAuthenticationToken(loginParams.getAuthId()));
            }
        }
        catch (Exception e)
        {
     ...
     * 微信注册
     */
    public String wxRegisterUser(RegisterBody registerBody)
    {
        String msg = "", username = registerBody.getUsername(), authId = registerBody.getAuthId();

            SysUser sysUser = new SysUser();
            sysUser.setAuthId(authId);
            sysUser.setUserName(username);
            sysUser.setNickName(username);
            boolean regFlag = userService.registerUser(sysUser);
            if (!regFlag)
