...
function run_svc_bin()
{
    ...
    if [[ $NODE_ENV == "dev" ]]; then
        NODE_HOST=${ip_address} NODE_PORT=${svc_port} CONSUL_HOST=localhost DATASOURCE_URL="jdbc:mysql://10.66.226.74:3306/${db_name}?useUnicode=true&characterEncoding=UTF-8&zeroDateTimeBehavior=convertToNull&allowMultiQueries=true&useSSL=false&serverTimezone=Asia/Shanghai" DATASOURCE_USER="sample_dbowner" DATASOURCE_PWD="TDkNWKSFatELG5pN" REDIS_HOST=localhost REDIS_PORT=6279 ${JAVA_CMD} -jar ${svc_bin} >> ${svc_log} 2>&1 &
    fi
